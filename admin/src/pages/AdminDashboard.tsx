...
import {
  ...
  AlertCircle, CheckCircle, Eye, XCircle, Truck, Loader2, Upload, Trash2, Shield, Star, RefreshCw,
...
export default function AdminDashboard() {
  // 从 localStorage 读取管理员角色，不依赖前台 AuthContext
  const adminUser = (() => {
    try {
      return JSON.parse(localStorage.getItem('3dprint_admin_user') || '{}')
    } catch { return {} }
  })()
                    ...
                    <tbody>
                      {allUsers.map(u => (
                        <tr key={u.id} className="border-b border-brand-border/50 hover:bg-brand-panel/50">
                          ...
                          <td className="py-3">
                            <span className={`px-2.5 py-1 text-xs rounded-full ${u.role === 'admin' ? 'bg-brand-blue/10 text-brand-blue' : 'bg-brand-text-dim/10 text-brand-text-dim'}`}>
                              {u.role === 'admin' ? '管理员' : '用户'}
                            </span>
                          </td>
                          ...
                          <td className="py-3">
                            {u.role !== 'admin' ? (
                              <button onClick={async () => {
                                setSettingRole(u.id)
                                try { await setUserRole(u.id, 'admin'); setAllUsers(allUsers.map(x => x.id === u.id ? { ...x, role: 'admin' } : x)) }
                                catch (e: any) { alert('操作失败') }
                                finally { setSettingRole(null) }
                              }} disabled={settingRole === u.id}
                                className="px-2 py-1 text-xs rounded disabled:opacity-50 bg-brand-blue/10 text-brand-blue hover:bg-brand-blue/20">
                                {settingRole === u.id ? '设置中...' : '设为管理员'}
                              </button>
                            ) : (
                              <button onClick={async () => {
                                setSettingRole(u.id)
                                try { await setUserRole(u.id, 'user'); setAllUsers(allUsers.map(x => x.id === u.id ? { ...x, role: 'user' } : x)) }
                                catch (e: any) { alert('操作失败') }
                                finally { setSettingRole(null) }
                              }} disabled={settingRole === u.id}
                                className="px-2 py-1 text-xs rounded disabled:opacity-50 bg-red-400/10 text-red-400 hover:bg-red-400/20">
                                {settingRole === u.id ? '取消中...' : '取消管理员'}
                              </button>
                            )}
                          </td>
                        </tr>
